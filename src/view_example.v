...
(* exo11 is taken from the map 2012 summer scool *)

Lemma exo11 : forall (P : nat -> Prop) (Q : Prop),
  (P 0 -> Q)-> 
  (forall n, P n.+1 -> P n) -> 
  P 4 -> Q.
Proof.
...
(* exo13 and exo14 are taken from the map 2012 summer school *)

(* use views only: move, case, with /orP, /andP and /(_ t) *)

Lemma exo12 (b b1 b2 : bool) : (b1 -> b) -> (b2 -> b) -> b1 || b2 -> b.
Proof.
Abort.

Lemma exo13 (Q : nat -> Prop) (p1 p2 : nat -> bool) x :
  ((forall y, Q y -> p1 y /\ p2 y) /\ Q x) -> p1 x && p2 x.
Proof.
Abort.

Lemma exo14 (Q : nat -> Prop) (p1 p2 : nat -> bool) x :
  ((forall y, Q y -> p1 y \/ p2 y) /\ Q x) -> p1 x || p2 x.
Proof.
Abort.
